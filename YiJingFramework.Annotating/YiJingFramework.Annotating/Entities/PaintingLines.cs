    ...
    IEqualityOperators<PaintingLines, PaintingLines, bool>
{
    ...
    /// </exception>
    public PaintingLines(Gua painting, Gua lines)
    {
    ...
    /// </exception>
    public PaintingLines(Gua painting, IEnumerable<int> linesIndexes, int indexBase = 0)
    {
        ArgumentNullException.ThrowIfNull(painting);
        ...
        this.Lines = new Gua(linesPainting);
    }
    ...
    /// <paramref name="linesIndexes"/> are out of range.
    /// </exception>
    public PaintingLines(Gua painting, params int[] linesIndexes)
        ...
        string s, [MaybeNullWhen(false)] out PaintingLines result)
    {
        ...
        var half = s.Length / 2;
        if (!Gua.TryParse(s[0..half], out var painting))
        {
            result = null;
            return false;
        }
    ...
    {
        return $"{this.Painting}{this.Lines}";
    }

    int IComparable<PaintingLines>.CompareTo(PaintingLines? other)
    {
        var c = this.Painting.CompareTo(other?.Painting);
        if (c is 0)
    ...
    {
        return this.Painting.Equals(other?.Painting) && this.Lines.Equals(other?.Lines);
    }
    ...
    {
        return HashCode.Combine(this.Painting, this.Lines);
    }
}